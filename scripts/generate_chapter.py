...
def publish_to_github(base, next_chapter_num, chapter_title):
    ...
    try:
        ...
        # Remove any AppleDouble resource forks that may have leaked onto exFAT
        ...
        # Stage changes in docs, public, scripts, readme, gitignore
        subprocess.run(['git', '-C', repo_dir, 'add', 'docs/', 'public/', 'scripts/'], check=True)
        if os.path.exists(os.path.join(repo_dir, 'README.md')):
            subprocess.run(['git', '-C', repo_dir, 'add', 'README.md', '.gitignore'], check=False)

        # Clean commit-tree approach avoids .git/COMMIT_EDITMSG on exFAT
        tree_res = subprocess.run(['git', '-C', repo_dir, 'write-tree'], capture_output=True, text=True, check=True)
        tree = tree_res.stdout.strip()

        parent_res = subprocess.run(['git', '-C', repo_dir, 'rev-parse', 'HEAD'], capture_output=True, text=True, check=True)
        parent = parent_res.stdout.strip()

        parent_tree_res = subprocess.run(['git', '-C', repo_dir, 'rev-parse', f'{parent}^{{tree}}'], capture_output=True, text=True, check=True)
        ...
        subprocess.run(['git', '-C', repo_dir, 'update-ref', 'refs/heads/main', commit], check=True)
        push_res = subprocess.run(['git', '-C', repo_dir, 'push', 'origin', 'main'], capture_output=True, text=True, check=True)
