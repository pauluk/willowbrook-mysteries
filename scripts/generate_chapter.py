...
def main():
    ...
    docs_html_path = os.path.join(base, 'willowbrook-mysteries/docs/index.html')
    safe_write_file(docs_html_path, html_content)

    docs_json_path = os.path.join(base, 'willowbrook-mysteries/docs/story_chronicles.json')
    safe_write_file(docs_json_path, json.dumps(book, indent=2))
    ...
    publish_to_github(base, next_chapter_num, new_chapter['title'])

def publish_to_github(base, next_chapter_num, chapter_title):
    """
    Publishes the updated novel reader and chronicles to GitHub using low-level
    git plumbing to avoid exFAT file locking on VirtioFS mounts.
    """
    import subprocess
    repo_dir = os.path.join(base, 'willowbrook-mysteries')
    try:
        print(f"📦 Publishing Chapter {next_chapter_num} to GitHub (willowbrook-mysteries)...")
        git_dir = os.path.join(repo_dir, '.git')

        # Remove any AppleDouble resource forks across the repository to prevent exFAT VirtioFS errors
        subprocess.run(['find', '.', '-name', '._*', '-delete'], cwd=repo_dir, capture_output=True)

        # Stage changes in docs, public, scripts, readme, gitignore explicitly
        items_to_add = [d for d in ['docs', 'public', 'scripts', '.gitignore', 'README.md'] if os.path.exists(os.path.join(repo_dir, d))]
        subprocess.run(['git', 'add'] + items_to_add, cwd=repo_dir, check=True)

        # Clean commit-tree approach avoids .git/COMMIT_EDITMSG on exFAT
        tree_res = subprocess.run(['git', 'write-tree'], cwd=repo_dir, capture_output=True, text=True, check=True)
        tree = tree_res.stdout.strip()

        # Safeguard: empty tree hash or unpopulated tree protection
        if not tree or tree == '4b825dc642cb6eb9a060e54bf8d69288fbee4904':
            print("⚠️ Safeguard triggered: Tree is empty; skipping commit to protect repository.")
            return

        parent_res = subprocess.run(['git', 'rev-parse', 'HEAD'], cwd=repo_dir, capture_output=True, text=True, check=True)
        parent = parent_res.stdout.strip()

        parent_tree_res = subprocess.run(['git', 'rev-parse', f'{parent}^{{tree}}'], cwd=repo_dir, capture_output=True, text=True, check=True)
        if tree == parent_tree_res.stdout.strip():
            print("ℹ️ No tree changes detected in willowbrook-mysteries; skipping commit.")
            return

        commit_msg = f"Chapter {next_chapter_num} update: {chapter_title} [Automated]"
        ...
        # Crucial for exFAT: remove any AppleDouble files created by macOS before update-ref
        subprocess.run(['find', '.', '-name', '._*', '-delete'], cwd=repo_dir, capture_output=True)

        subprocess.run(['git', 'update-ref', 'refs/heads/main', commit], cwd=repo_dir, check=True)
        push_res = subprocess.run(['git', 'push', 'origin', 'main'], cwd=repo_dir, capture_output=True, text=True, check=True)
        print(f"✓ Successfully published Chapter {next_chapter_num} to GitHub:\n{push_res.stderr.strip() or push_res.stdout.strip()}")
    except Exception as e:
        print(f"⚠️ Warning: Automatic git publish failed (non-fatal): {e}")
